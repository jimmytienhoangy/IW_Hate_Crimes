...
Original file is located at
    https://colab.research.google.com/drive/1j27ZJqsk6sswVXLoTSRCv95Ao9kYJlsJ
...
logistic = LogisticRegression(multi_class='multinomial', penalty = 'l1', solver = 'saga')
...
print("Balanced Accuracy Score: ", balanced_accuracy_score(y_test, y_pred_logistic))

disp = ConfusionMatrixDisplay.from_predictions(y_test, y_pred_logistic,
cmap=plt.cm.Blues, normalize = 'true', display_labels=bias_groups_str,
xticks_rotation = 'vertical')

fig = disp.figure_
fig.suptitle('Multiclass Logistic Regression Untuned')

"""LR Tuning (Weighted F1)"""

base_logistic = LogisticRegression()

logistic_space = {'multi_class': ['multinomial'],
            'penalty': ['l1'],
            'solver': ['saga'],
             'C': [1.0, 0.5, 0.1]}

gs_logistic = GridSearchCV(base_logistic, logistic_space, cv = 5, n_jobs = -1, scoring = 'f1_weighted')

gs_logistic.fit(X_train, y_train)

print('Best Weighted F1: %.2f' % gs_logistic.best_score_)
print('Best Parameters: ', gs_logistic.best_params_)

y_pred_logistic_cv = gs_logistic.predict(X_test)

print(classification_report(y_test, y_pred_logistic_cv, digits = 4))

disp = ConfusionMatrixDisplay.from_predictions(y_test, y_pred_logistic_cv,
cmap=plt.cm.Blues, normalize = 'true', display_labels=bias_groups_str, xticks_rotation = 'vertical')

fig = disp.figure_
fig.suptitle('Multiclass Logistic Regression Tuned for Weighted F1')

"""LR Tuning (Balanced Accuracy)"""
...
gs_logistic = GridSearchCV(base_logistic, logistic_space, cv = 5, n_jobs = -1, scoring = 'balanced_accuracy')

gs_logistic.fit(X_train, y_train)

print('Best Balanced Accuracy: %.2f' % gs_logistic.best_score_)
print('Best Parameters: ', gs_logistic.best_params_)

y_pred_logistic_cv = gs_logistic.predict(X_test)

print(classification_report(y_test, y_pred_logistic_cv, digits = 4))
print("Balanced Accuracy Score: ", balanced_accuracy_score(y_test, y_pred_logistic_cv))
...
fig = disp.figure_
fig.suptitle('Multiclass Logistic Regression Tuned for Balanced Accuracy')