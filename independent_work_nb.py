...
Original file is located at
    https://colab.research.google.com/drive/1keW-EeViAjVdJ2iwl4FTqoGAuj4gwNpB
...
print("Balanced Accuracy Score: ", balanced_accuracy_score(y_test, y_pred_dummy))

disp = ConfusionMatrixDisplay.from_predictions(y_test, y_pred_dummy,
cmap=plt.cm.Blues, normalize = 'true', display_labels=bias_groups_str,
xticks_rotation = 'vertical')

fig = disp.figure_
fig.suptitle('Baseline: Uniform Dummy Classifier')
...
print("Balanced Accuracy Score: ", balanced_accuracy_score(y_test, y_pred_nb))
...
fig = disp.figure_
fig.suptitle('Naive Bayes Untuned')

"""Naive Bayes Tuning (Weighted F1)



"""

base_nb = CategoricalNB()

nb_space = {'alpha': [0.0, 0.1, 0.5, 1.0],
           'force_alpha': [True, False]}

gs_nb = GridSearchCV(base_nb, nb_space, cv = 5, n_jobs = -1, scoring = 'f1_weighted')

gs_nb.fit(X_train, y_train)

print('Best Weighted F1: %.2f' % gs_nb.best_score_)
print('Best Parameters: ', gs_nb.best_params_)

y_pred_nb_cv = gs_nb.predict(X_test)

print(classification_report(y_test, y_pred_nb_cv, digits = 4))

disp = ConfusionMatrixDisplay.from_predictions(y_test, y_pred_nb_cv,
cmap=plt.cm.Blues, normalize = 'true', display_labels=bias_groups_str,
xticks_rotation = 'vertical')

fig = disp.figure_
fig.suptitle('Naive Bayes Tuned for Weighted F1')

"""Naive Bayes Tuning (Balanced Accuracy)"""
...
gs_nb = GridSearchCV(base_nb, nb_space, cv = 5, n_jobs = -1, scoring = 'balanced_accuracy')

gs_nb.fit(X_train, y_train)

print('Best Balanced Accuracy: %.2f' % gs_nb.best_score_)
print('Best Parameters: ', gs_nb.best_params_)

y_pred_nb_cv = gs_nb.predict(X_test)

print(classification_report(y_test, y_pred_nb_cv, digits = 4))
print("Balanced Accuracy Score: ", balanced_accuracy_score(y_test, y_pred_nb_cv))
...
fig = disp.figure_
fig.suptitle('Naive Bayes Tuned for Balanced Accuracy')